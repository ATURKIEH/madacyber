...
def main():
    ...
    wordlist_path = "/opt/seclists/SecLists/Discovery/DNS/bug-bounty-program-subdomains-trickest-inventory.txt"
    ...
    if os.path.exists(final_combined_filename) and os.path.getsize(final_combined_filename) > 0:
        ...
        if not os.path.exists(gospider_exe_path):
            print(f"[!] GoSpider executable not found: '{gospider_exe_path}'. Skipping GoSpider.")
        else:
            try:
                ...
                gospider_runner.run_on_all(original_target_domain_for_scoping=domain_input)
    ...
    if os.path.exists(final_combined_filename) and os.path.getsize(final_combined_filename) > 0:
        print(f"\n--- Preparing for GAU Scans (Input from: {final_combined_filename}) ---")
        ...
        print("\n[*] Skipping GAU: Input file for GAU '{final_combined_filename}' is empty or not created.")
    print("\n--- Finished GAU Stage ---")
